...
if(isset($_GET["Adopter_ID"]) && !empty(trim($_GET["Adopter_ID"]))){
  ...
  //  $Adopter_ID = $_SESSION["Adopter_fname"];
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($fname_err) && empty($lname_err) && empty($DOB_err) && empty($street_err) && empty($city_err) && empty($state_err) && empty($zip_err)){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            mysqli_stmt_bind_param($stmt, "sssdis", $param_fname, $param_lname,$param_DOB, $param_street,$param_city, $param_state, $param_zip, $param_Adopter_ID);
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URL'])); ?>" method="post">
                        ...
                        <div class="form-group <?php echo (!empty($zip_err)) ? 'has-error' : ''; ?>">
                            <label>City</label>
                            <input type="number" min="1" max="20" name="Zip" class="form-control" value="<?php echo $Zip; ?>">
